  ...
  if (isOffline) {
    return (
      <div className="translation-panel">
        <div className="error-state offline-state">
          <span className="error-icon">🔌</span>
          <p>You are offline. Translation requires an internet connection.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="translation-panel">
        <div className="error-state">
          ...
          <p>{error}</p>
          {errorCode === 'INVALID_JSON' && onRetry && (
        ...
        {result.entries.map((entry, idx) => (
          <div key={idx} className="dictionary-entry">
            <div className="entry-header">
              {entry.word && <span className="entry-word">{entry.word}</span>}
              {entry.phonetic && <span className="entry-phonetic">{entry.phonetic}</span>}
              {entry.partOfSpeech && (
